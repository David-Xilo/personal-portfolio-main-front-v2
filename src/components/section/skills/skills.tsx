...
export default function Skills() {
    ...
    // Dynamic helper to match the monospaced slugs from the original design
    ...
    return (
        <section id="skills" className="py-[100px] border-t border-dm-line dark:border-dm-line-dark scroll-mt-16">
            <div className="max-w-[1180px] mx-auto px-6 md:px-16">
                ...
                {/* Skills Grid */}
                {status === 'success' && skillCategories && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {skillCategories.map((cat, idx) => (
                            <div 
                                key={idx} 
                                className="p-6 border border-dm-line dark:border-dm-line-dark rounded-xl bg-dm-bg-elev/30 dark:bg-dm-bg-elev-dark/30 hover:bg-dm-bg-elev/60 dark:hover:bg-dm-bg-elev-dark/60 hover:border-dm-accent dark:hover:border-dm-accent-dark transition-all duration-300 flex flex-col gap-6"
                            >
                                {/* Cell Header */}
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {cat.skills.map((skill, sIdx) => (
                                        <span 
                                            key={sIdx}
                                            className="font-mono text-[11px] tracking-wide px-2.5 py-[5px] bg-dm-bg-elev dark:bg-dm-bg-elev-dark/40 border border-dm-line dark:border-dm-line-dark rounded text-dm-text-mut dark:text-dm-text-mut-dark cursor-default transition-all duration-200 hover:border-dm-accent dark:hover:border-dm-accent-dark hover:text-dm-text dark:hover:text-dm-text-dark"
                                        >
                                            {skill}
                                        </span>
                                    ))}
